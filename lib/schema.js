...
schemas.Parameter = Joi.object({
  ...
  // else
  type: Joi.alternatives().conditional("in", { is: "body", otherwise: dataTypes.required() }),
  format: Joi.alternatives().conditional("in", { is: "body", otherwise: dataFormat.optional() }),
  items: Joi.alternatives().when("type", { is: "array", then: Joi.alternatives().try(schemas.SimpleReference, schemas.Item).required() }),
  collectionFormat: Joi.alternatives().conditional("in", {
    is: "body",
    otherwise: Joi.string().valid("csv", "ssv", "tsv", "pipes", "multi").default("csv").optional()
  }),
  default: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.any().optional() }),
  maximum: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.number().optional() }),
  exclusiveMaximum: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.boolean().optional() }),
  minimum: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.number().optional() }),
  exclusiveMinimum: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.boolean().optional() }),
  maxLength: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.number().integer().optional() }),
  minLength: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.number().integer().optional() }),
  pattern: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.string().optional() }),
  maxItems: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.number().integer().optional() }),
  minItems: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.number().integer().optional() }),
  uniqueItems: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.boolean().optional() }),
  enum: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.any().optional() }),
  multipleOf: Joi.alternatives().conditional("in", { is: "body", otherwise: Joi.number().optional() })
}).meta({
  className: "Parameter"
})
