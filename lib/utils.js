...
utils.generateNameFromSchema = function (schema) {
  ...
  const isPrimitive = schema && utils.isPrimitiveSwaggerType(schema.type)
  let keys = []
  ...
  if (isPrimitive) {
    ...
    const children = Object.fromEntries(schema._ids._byKey)
    keys = _.map(_.map(children, "id"), _.upperFirst)
  }
...
utils.isSupportedSchema = function (schema) {
  return schema != null && !(schema instanceof Function) && _.get(schema, "_flags.func") !== true && Joi.isSchema(schema) === true && (utils.isSupportedType(utils.getPrimitiveType(schema)) || utils.getMetaSwaggerType(schema) != null)
}
...
utils.parseBaseModelAttributes = function (schema) {
  ...
  utils.setNotEmpty(baseModel, "example", schema.$_terms.examples && schema.$_terms.examples[0] || "")
...
utils.getFirstInclusionType = function (schema) {
  const inclusionTypes = _.get(schema, "$_terms._inclusions")
